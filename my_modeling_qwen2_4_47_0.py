...
class Qwen2Attention(nn.Module):
    ...
    def forward(
        ...
        global_attn_weights = None
        if output_attentions and global_attn_weights is None:
            global_attn_weights = attn_weights.mean(dim=1)  

        return attn_output, global_attn_weights, past_key_value
...
class Qwen2DecoderLayer(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        if not output_attentions: o = False
        elif self.layer_idx not in SELECTED_LAYERS: o = False
        else: o = True
        # Self Attention
...
class Qwen2Model(Qwen2PreTrainedModel):
    ...
    def forward(
        ...
        if output_attentions:
            ...
            causal_mask_eager = self._update_causal_mask(
                attention_mask, inputs_embeds, cache_position, past_key_values, output_attentions
            )
            self.config._attn_implementation = attn
        ...
        for layer_idx, decoder_layer in enumerate(self.layers[: self.config.num_hidden_layers]):
            ...
            if output_attentions and layer_idx in SELECTED_LAYERS:
                c = causal_mask_eager
            else: c = causal_mask
            if self.gradient_checkpointing and self.training:
